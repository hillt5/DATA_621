...
```
We have imposed a linear fit (with 95% confidence band in grey) to each of the pairs of variables to understand the relationship.There does not seem to be a strong linear relationship between any of the pairs (strongest appear to be with temperature). We can also see some outliers in the data. Please note, that there are instances where the response variable seems to fall below 0 - such as for area and FFMC. Once again, this may mean that the burned area was below 1 ha.
...
If we get rid of fires with area of 0, then we get an R^2 of 19%
```{r model-4}
...
One thing to note from our linear models, is the difference between the adjusted r^2 and r^2 - this indicates that there is multicollinearity in our data (which we has observed with some of the IVs during EDA). This may result in overfitting i.e. overestimating some of the coefficients assigned to our predictors. Such a model, will not perform well in the unseen test data. Therefore, we perform regularization through ridge regression to overcome this issue.
...
```{r,ridge}
library(glmnet)
x = as.matrix(X.train) # we need to convert our dataframes to a matrix to perform regularization
y_train = X.train$area # highlighting the dependent variable
...
eval_results(y_test, predictions_test, X.test)
```

We can see that our r^2 for the regularized model is 0.99 and the RMSE between the the actual and predicted number is very low, for both the training and test data sets.
