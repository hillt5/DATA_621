...
It seems like our dataset has 517 observations and 13 columns. None of the variables have any missing values.
It is also interesting to note that our response variable, area, has a large range and the difference between the median (0.52) and mean (~12) seems to suggest there maybe outliers in our data. It also seems like there are a number of 0s in our response variable. Let's check this:
...
We can see in above 2 plots that fire frequency is more on weekends. Also Aug and Sep are the months having most of the fire in the year.



## Numeric Variables

Except for the day and month columns, all of these variables are numeric.  The original authors mention that our dependent variable, area is likely to require log or some other transformation for best contribution to a model.
...
It seems like our dataset has multicollinearity between the independent variables. Let's also further look ar the relationship between our response variable (area), and each of the predictor variables.
```{r,corr2}
...
```
We have imposed a linear fit (with 95% confidence band in grey) to each of the pairs of variables to understand the relationship. We can also see some outliers in the data. Please note, that there are instances where the response variable seems to fall below 0 - such as for area and FFMC. Once again, this may mean that the burned area was below 1 ha.

## Outliers

We suspect outliers in some of the variables. Let's further test the theory out:
...
Variables such as RH, FFMC, DMC and DC seem to have outliers in the data.
...
Here we are dropping the variable, **rain**.
```{r mice-impute}
set.seed(317)
...
Finally in this step for data preparation we will partition the training dataset for training and test using `createDataPartition` method from `caret` package. We will reserve 75% for training and rest 25% for validation purpose.
...
Using all data gives R^2 of ~10%
```{r, model1}
...
Using stepAIC() function gives us R^2 of 6%
```{r, model 2}
stepAIC_lm <- stepAIC(lm1,direction='both')
summary(stepAIC_lm)
...
This subset only runs the linear model for small forest fires.
...
If we get rid of fires with area of 0, then we get an R^2 of 19%. While we have a large number of records where area=0, this subset would be interesting to examine if the fire department is looking to predict large forest fires that would need substantial resources to deal with.
...
leaps() performs an exhaustive search for determing the best predictors to use to predict our target variable.
...
```
Seems like we have to use 13 predictors to get the best mode and the summary table below provoides details on which predictors to use for the model. The best predictors are indicated by 'TRUE'.
```{r,which-predictors}
...
Our dataset also had outliers - which we handled through imputation during data preparation. However, we are also going to build a robust regression model to fully ensure that outliers and influential observations are not affecting our model. Robust regression is an iterative procedure that seeks to identify outliers and
minimize their impact on the coefficient estimates.
...
Upon comparing the RSE from our original linear model (best.model) and the robust regression model, keeping all variables constant, it seems that our original model performance was better.

From here on, we will dismiss the best model produced by leaps and move forward with the subset where burned area is greater than 0 ha. 

However, upon revising the coefficients of our model, it seems like the only signficant predictor for large fires at p<.05 level is DMC - a moisture code index that takes rates moisture content that takes into account rain, humidity and temperature. We will therefore try to refine this model a little bit with the leaps package to see if we can get a better fit just for this subset.
...
```
Seems like we have to use 12 predictors to get the best mode and the summary table below provoides details on which predictors to use for the model. The best predictors are indicated by 'TRUE'.
```{r,which-predictors2}
...
summary(lm0edit)
```
Our adjusted r^2 has decreased slightly, however we are seeing several significant predictors. We had flagged the dataset for multicollinearity previously and upon running the car() package to test for multicollinearity in our chosen regression mode - it seemed like there were aliased coefficients in the model suggesting perfect collinearity between certain IVs. Therefore, despite the lower adjusted r^2, it maybe safer to move forward with this model to avoid overfitting.
...
Ridge regression is an extension of linear regression where the loss function is modified to minimize the complexity of the model (Singh,2019). We will build the glmnet() package to build the regularized model.
...
Before finalizing the linear regression model for predicting large forest fires, we will analyze the diagnostic plots for our chosen model.
