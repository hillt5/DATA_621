...
First, let's look at the multiple linear regression using all variables.  The R^2 value is very low, around 4%, and many of the variables are not significant.
...
```{r multiple-linear-regression-backward-elim}
...
Here we will assume our model has 13 parameters and this gives us the following equation:
```{r mlr_coeff-2}
coef(mlr_full,13)
...
### Multiple Regression Using Leaps

Using the leaps package on one of the transformed datasets, we will try to find the best model.

```{r,Linear}

insurance_LINEAR <- regsubsets(TARGET_AMT ~ . ,  data = insurance_fix2,
...
```{r,best-model}
best.model <- lm(TARGET_AMT~MSTATUS+CAR_TYPE+OLDCLAIM+TARGET_FLAG+PARENT1+BLUEBOOK+YOJ+HOME_VAL+REVOKED, data =insurance_fix2 )
summary(best.model)
```
## Model Selection
