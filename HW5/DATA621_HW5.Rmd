...
summary(glm.nb(wine_train1, formula = TARGET ~. ))
...
p1 <- c('STARS = 0 where missing',poisson1$aic,get_mse(poisson1))
...
```
Here we see Poisson #1 is the best model because it has the lowest AIC and MSE.  
...
predictions_rounded <- ceiling(predictions)
table(predictions_rounded)
```

```{r, plotted-test-predictions}
hist(predictions_rounded)
